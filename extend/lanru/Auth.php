...
class Auth
{
    ...
    public function getGroups($uid)
    {
        ...
        $user_groups = Db::name($this->config['auth_group_access'])
            ->alias('aga')
            ->join('__' . strtoupper($this->config['auth_group']) . '__ ag', 'aga.admin_group_id = ag.id', 'LEFT')
            ->field('aga.admin_id,aga.admin_group_id,ag.id,ag.pid,ag.name,ag.rules')
    ...
    public function getRuleList($uid)
    {
        ...
        //读取用户组所有权限规则
        $this->rules = Db::name($this->config['auth_rule'])->where($where)->field('id,pid,icon,name,rule,ismenu')->select();
