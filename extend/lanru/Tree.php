    ...
    public function getTreeMenu($myid, $itemtpl, $selectedids = '', $disabledids = '', $wraptag = 'ul', $wrapattr = '', $deeplevel = 0)
    {
        ...
        if ($childs) {
            foreach ($childs as $value) {
                ...
                $value = array(
                    '@childlist' => $childlist,
                    '@url'       => $childdata || !isset($value['@url']) ? "" : " href='" . url($value['@url']) . "'",
                    '@caret'     => ($childdata && (!isset($value['@badge']) || !$value['@badge']) ? '<span class="caret"></span>' : ''),
                    '@badge'     => isset($value['@badge']) ? $value['@badge'] : '',
                    '@class'     => ($selected ? ' active' : '') . ($disabled ? ' disabled' : '') . ($childdata && $value['@isopen'] == 1 ? 'cm-submenu pre-open' : ($childdata ? ' cm-submenu' : '') ),
                    '@title'     => $value['@title'],
                );
